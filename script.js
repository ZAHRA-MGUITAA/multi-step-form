function adderrormessage() {
  ...
  name.addEventListener("keyup", () => {
    if (name.value.trim() === "" && !namerrorAdded) {
      ...
      // name.insertAdjacentHTML("beforebegin", errorname);
      document
...
function switch_() {
  ...
  if (toggle.classList.contains("monthly")) {
    ...
    // document.querySelectorAll("span.planprice.yearly").style.display = "block";
...
const sidebarbutton = () => {
  ...
  step2.addEventListener("click", () => {
    document.querySelector("#step-2").style.display = "grid";
    document.querySelector("#step-3").style.display = "none";
    document.querySelector("#step-4").style.display = "none";
    ...
    planstep2.forEach((plan) => {
      if (plan.checked) {
        saveToLocalStorage("plan", {
          planname: plan.value,
          planprice:
            plan.nextElementSibling.querySelector(".planprice.monthly").innerHTML,
        });
      }
    });
    ...
    planstep2.forEach((plan) => {
      if (plan.checked) {
        saveToLocalStorage("plan", {
          planname: plan.value,
          planprice:
            plan.nextElementSibling.querySelector(".planprice.yearly").innerHTML,
        });
      }
    });
...
function thirdstep() {
  ...
  let toggle = document.querySelector(".toggle-switch");
  // if (toggle.classList.contains("monthly")) {
  //   document.querySelectorAll('.price.monthly').forEach((m)=>{
  //     m.style.display = "none";
  //     m.nextElementSibling.style.display = "block";
  //   })
  // }else if (toggle.classList.contains("yearly")) {
  //   document.querySelectorAll('.price.yearly').forEach((m)=>{
  ...
  addons.forEach((addon) => {
    if (toggle.classList.contains("monthly")) {
      addon.nextElementSibling.nextElementSibling.nextElementSibling.style.display = "none";
      ...
      addon.nextElementSibling.nextElementSibling.nextElementSibling.style.display = "block";
      // console.log(addonprice);
    }
    if (addon.checked) {
      if (toggle.classList.contains("monthly")) {
        ...
        let addonvalue = addon.value;
        let addonprice = addon.nextElementSibling.nextElementSibling.nextElementSibling.innerHTML;
        // addon.nextElementSibling.nextElementSibling.style.display = "none";
...
function forthstep() {
  ...
  selectedaddons.forEach((addon) => {
    ...
    //   serviceonline.append(`<div>
    //   <p>${addon.value}</p>
    //   <p>${addon.price}</p>
    // </div>`)
  });
  total.innerHTML = "+$" + totalprice + (toggle.classList.contains('monthly')?"/mo":"/yr");
}
